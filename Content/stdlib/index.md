...
[namespace](xref:language.namespaces) decides what it is *called*. Most of the
...
[loops](xref:language.control.loops), and carry the operators
[below](#sequences).

## Sequences

`Enumerable<T>` carries seven operators as
[members the interface supplies](xref:language.interfaces#members-the-interface-supplies),
...
Each takes a [lambda](xref:language.lambdas), and `map` and `filter` answer
...
Both carry the [`noreturn`](xref:language.attributes#noreturn) attribute, so a
...
Two global [unions](xref:language.unions), for a value that may be absent and
...
`map`, `and_then` and `filter` take a [lambda](xref:language.lambdas) and leave
